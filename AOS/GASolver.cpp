...
int getRandomNumber(int min, int max) { return (rand() % max) + min; }
...
Population::Population(unsigned int SizePop, unsigned int SizeGenes,
                       InitPopType Type) {
  Generations = 0;
  Size = SizePop;
...
std::vector<std::string> GASolver::Solve(llvm::Module *M) {
  ...
  case 1:
    Pop = std::make_shared<Population>(Params.populationSize, Params.max,
                                       InitPopType::BASELINE);
    break;
  case 2:
    Pop = std::make_shared<Population>(Params.populationSize, Params.max,
                                       InitPopType::BEST10);
