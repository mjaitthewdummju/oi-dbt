...
// Run perf record
// then perf report --stdio | sed '/^#/d' | awk '{print $1,$3,$5}' | sed
// '/^\s*$/d' | grep "a.out" | sed 's/%//g'

unsigned NumOfInsts = 0;
void MethodBased::addFunctionToCompile(uint32_t PC, Machine &M) {
  if (AlreadyCompiled.count(PC) != 0)
    return;
  if (NumOfInsts > RegionLimitSize)
    return;
  ...
  if (M.isMethodEntry(PC)) {
    ...
    for (uint32_t Addr = PC; Addr < M.getMethodEnd(PC); Addr += 4) {
      ...
      if (I.Type == OIDecoder::OIInstType::Call ||
          I.Type == OIDecoder::OIInstType::Syscall ||
          I.Type == OIDecoder::OIInstType::Callr ||
          I.Type == OIDecoder::OIInstType::Ijmp)
    ...
    for (auto PossibleEntry : PossibleEntries) {
      ...
      for (uint32_t Addr = PossibleEntry; Addr < M.getMethodEnd(PC);
           Addr += 4) {
