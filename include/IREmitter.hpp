...
private:
  ...
  void addMultipleEntriesSupport(std::vector<uint32_t> &, llvm::BasicBlock *,
                                 llvm::Function *);
...
public:
  enum RegType { Int, Float, Double, Int64 };
  ...
  llvm::Value *genDataVecPtr(llvm::Value *, llvm::Function *, llvm::Type *,
                             unsigned);
  ...
  llvm::Value *genLoadRegister(uint16_t, llvm::Function *,
                               RegType Type = RegType::Int);
  llvm::Value *genStoreRegister(uint16_t, llvm::Value *, llvm::Function *,
                                RegType Type = RegType::Int);
  ...
  IREmitter() { Builder = std::make_unique<llvm::IRBuilder<>>(TheContext); };

  void generateRegionIR(std::vector<uint32_t> &, const OIInstList &, uint32_t,
                        dbt::Machine &, llvm::TargetMachine &,
                        volatile uint64_t *NativeRegions, llvm::Module *);
  ...
  static size_t getAssemblySize(const void *func) {
    ...
    LLVMDisasmContextRef D =
        LLVMCreateDisasm(LLVM_HOST_TRIPLE, NULL, 0, NULL, NULL);
  ...
  static size_t disassemble(const void *func, std::ostream &buffer) {
    ...
    for (pc = 0; pc < extent; pc += Size) {
      ...
      buffer << "<" << std::hex << static_cast<const void *>(bytes) << "+"
  ...
  static size_t regionDump(const void *func, std::ostream &buffer,
                           size_t size) {
    const uint8_t *bytes = (const uint8_t *)func;
